    ...
    <section className="max-w-6xl mx-auto px-6 py-20">
      ...
      >
        ...
        <p className="mt-4 text-lg text-text/60 font-medium max-w-lg mx-auto">
          Aplikacja jest w fazie testów. Wpisz swój e-mail, a damy Ci znać, gdy będziesz mógł ją
          wypróbować.
        </p>
      </motion.div>
        ...
        {status === 'success' ? (
          <div className="py-4">
            ...
            <p className="text-text/60">
              Dziękujemy! Damy Ci znać, gdy Hopety będzie gotowe do testów.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} noValidate>
            ...
            {status === 'error' && (
              <p className="mt-2 text-sm text-red-500 text-left">{errorMessage}</p>
            )}

            <button
              type="submit"
              disabled={status === 'loading'}
              className="mt-3 w-full bg-primary text-white rounded-xl px-8 py-3 font-semibold hover:bg-primary/90 transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
            >
              {status === 'loading' ? (
                <>
